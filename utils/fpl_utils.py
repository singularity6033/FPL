...
from utils.basic import one_hot_embedding, data_randomize, gain_schedule_old, gain_schedule, my_data_loader
...
# this function calculates the 2nd term in (73)
...
def inc_solve_2_layer_conv_fc(epoch_no, batch_no, in_image, out_image, pool_layer='max', fil=None, fc_wei=None,
                              fun_front=f.relu, fun_after=mf.my_identity, loop=1, mix=False, stride=1, pad=0, gain=0.01,
                              auto=True):
...
def inc_train_2_layer(model, train_loader, test_loader, pool_layer='max', epochs=2000, gain=0.01, auto=True, true_for=1,
                      model_name='_0', avg_N=0):
    # input layer --> conv layer --> pooling layer --> flatten layer --> fc layer
    # input layer --> conv layer --> flatten layer --> fc layer
    if pool_layer:
    ...
    with torch.no_grad():
        ...
        for j in range(epochs):
            ...
            print('time for epoch', j + 1, '/', epochs, ': ', time.time() - t1, 's')
    ...
    with torch.no_grad():
        ...
        for j in range(epoch):
            ...
            for i, (x, y) in enumerate(train_loader):
                ...
                layer_tar = one_hot_embedding(y.long(), model.no_outputs).to(DEVICE_[0]).float()
                v, w, alpha_w_, lr = inc_solve_2_fc_layer(model, train_loader, test_loader, i, False, layer_in,
                                                          layer_tar,
